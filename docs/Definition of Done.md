...
50 циклов power on/off без “матрица не включилась”.

10 циклов OTA_START + timeout без зависаний и с корректным возвратом.

M1. DOA: XVF → ESP (без голоса, только телеметрия)

Цель: ESP получает стабильный угол 0…360°.

M1.1 Реализация чтения DOA

Использовать XVF control параметр AEC_AZIMUTH_VALUES, брать Auto selected beam.

Вывести угол в градусах (0…360), определить ноль/направление (соглашение координат).

M1.2 Стабилизация

Deadband (например ±5°) + сглаживание (EMA/median).

Выход: doa_deg_filtered + doa_valid.

✅ Приёмка M1:

При голосе слева/справа угол меняется адекватно, без “дребезга лица”.

M2. Audio IN: XVF → ESP (под wake/ASR)

Цель: непрерывный аудиопоток на ESP для wake и команд.

M2.1 Стандартизация формата

Вход сейчас: 16 kHz, 2ch, 24-in-32.

Для wake/ASR приводим к: 16 kHz mono s16 (downmix/выбор канала + shift/saturate).

M2.2 Буферизация и устойчивость

Ringbuffer, контроль overruns/underruns.

Отладочный VAD/уровень энергии.
