...
### P1.x FIRE: плавность при New Time Approach

Симптом:
- при FPS ≥ 25 эффект FIRE периодически выходит за frame budget,
- наблюдаются микродёрги, несмотря на корректную модель времени.

Причина:
- дискретная симуляция с пороговым step_ms,
- неравномерное распределение sim-шагов по кадрам.

Требование:
- **не снижать визуальное качество огня**,
- допускается только оптимизация или улучшение алгоритма.

Варианты решения (позже):
- оптимизация симуляции,
- уменьшение порогового эффекта step,
- интерполяция состояний между шагами.
