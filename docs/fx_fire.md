...
Эффект FIRE успешно переведён на новую модель времени (anim-time).

Текущее состояние:
- корректно реагирует на pause/speed,
- визуально стабилен,
- работает в рамках нового FX-контракта.

Известный технический долг:
- при высоком FPS (≈25) симуляция FIRE упирается в CPU-бюджет,
- возможны микродёрги из-за дискретного sim-step.

План улучшения (без снижения визуального качества):
- оптимизация симуляции,
- уменьшение пороговых эффектов step-based обновления,
- возможная интерполяция между состояниями.
...
Сделано:
- Исправлена сборка после правок TIP-профиля (пропавшие локальные переменные в `fire_render_field`).
- Проведена оптимизация без изменения визуального эталона “скорости” огня.
- Принято решение по FPS: **22 FPS** как production default.

Инварианты FIRE (зафиксированы):
- `FIRE_W = 16` — ширина поля фиксирована (power-of-two), это используется для оптимизаций wrap/доступа.
- Визуальная скорость текущей версии FIRE — эталон (любые future-твики скорости должны быть opt-in через define и дефолтом не менять поведение).

Производительность (наблюдение):
- При 22 FPS: система работает в бюджете (редкие miss возможны из-за системных всплесков).
- При попытке 25 FPS: эффект упирается в физическое время `show` (≈22.5 ms) и становится хрупким по бюджету/потерям пакетов.
